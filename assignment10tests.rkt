...
(valid-program? (arith '* (bool #f) (bool #t)))
...
(not (valid-program? (comp '<< (num 3) (num 5))))
(not (valid-program? (comp '>= (num 'a) (bool #f))))
(not (valid-program? (if-e (var '#f) (bool #f) (num 5))))
(not (valid-program? (if-e (var 'f) (bool 'f) (num 5))))
(not (valid-program? (if-e (var 'f) (bool #f) (num 'd5))))
(valid-program? (if-e (var 'f) (bool #f) (num 5)))
(valid-program? (if-e (comp '< (num 5) (bool #f)) (arith '+ (var 'd) (bool #t)) (nul)))
...
(not (valid-program? (fun #f 't (bool 'f))))
...
(value? (nul))
(value? (clos (var 'dd) (num 'd)))
(value? (bool (clos 'd 'f)))
(not (value? (comp '< (num 5) (num 7))))
(not (value? (arith '+ (num 4) (num 7))))
(not (value? (let-e 's (var 'e) (num 5))))
(not (value? (if-e (bool #t) (num 5) (var 'd))))
...
(value-eq? (nul) (nul))
(not (value-eq? (isnul (nul)) (isnul (nul))))
...
(equal? (evaluate (arith '* (num 3) (num 2)))
        (num 6))
(with-handlers ([exn:fail? (lambda (exn) #t)])
  (equal? (evaluate (var 'd))
          (nul)))
(equal? (interp sample-env1 (var 'x))
        (num 4))
...
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (evaluate (comp '< (num 3) (num 2)))
          (bool #f)))
(with-handlers ([exn:fail? (lambda (exn) #t)])
  (equal? (evaluate (arith '+ (bool #t) (num 2)))
          (3)))
(with-handlers ([exn:fail? (lambda (exn) #t)])
  (equal? (evaluate (comp '< (bool #t) (num 2)))
          (3)))
(with-handlers ([exn:fail? (lambda (exn) #t)])
  (equal? (interp sample-env1 (if-e (var 'x) (num 3) (num 2)))
          (3)))
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (interp sample-env1 (if-e (comp '< (num 3) (num 2)) (num 3) (num 2)))
          (num 2)))
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (interp sample-env1 (eq-e (var 'x) (num 4)))
          (bool #t)))
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (interp sample-env1 (eq-e (var 'y) (num 4)))
          (bool #f)))
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (interp sample-env1 (let-e 's (var 'y) (arith '+ (var 'y ) (num 4))))
          (num 9)))
(with-handlers ([exn:fail? (lambda (exn) #f)])
...
(with-handlers ([exn:fail? (lambda (exn) #f)])
  (equal? (evaluate (call (fun 'f 'x (if-e (eq-e (var 'x) (num 0))
          ...
          (num 3)))
(equal? (evaluate (isnul (nul)))
        (bool #t))
(equal? (evaluate (isnul (num 3)))
        (bool #f))
(equal? (evaluate (isnul (call (fun 'd 'x (nul)) (num 3))))
        (bool #t))
(equal? (evaluate (isnul (call (fun 'd 'x (nul)) (bool #t))))
        (bool #t))
(equal? (interp sample-env1 (isnul (call (fun 'd 'x (nul)) (var 'x))))
        (bool #t))
(equal? (interp sample-env1 (pair-e (call (fun 'd 'x (nul)) (var 'x)) (var 'x)))
        (pair-e (nul) (num 4)))
(equal? (interp sample-env1 (fst (pair-e (call (fun 'd 'x (nul)) (var 'x)) (var 'x))))
        (nul))
(equal? (interp sample-env1 (snd (pair-e (call (fun 'd 'x (nul)) (var 'x)) (var 'x))))
        (num 4))
;; neq
(displayln "neq tests")
(with-handlers ([exn:fail? (lambda (exn) #f)])
